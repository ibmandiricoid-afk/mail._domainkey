          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-[0_12px_36px_-6px_rgba(15,23,42,0.12)] flex-1 flex flex-col min-h-[460px] overflow-hidden">
            ...
            <div className="px-2.5 sm:px-3.5 py-2 border-b border-slate-200 bg-slate-100/70 flex flex-col gap-1 relative shrink-0 rounded-t-2xl">
              <div className="flex justify-between items-center gap-2">
                <h2 className="text-[10px] font-extrabold text-slate-400 uppercase tracking-widest flex items-center gap-1.5 whitespace-nowrap min-w-0">
                  ...
                  <span className="truncate">Sistem Anti-Spam Gmail</span>
                </h2>
              ...
              <div>
                {smtpConfig.username ? (
                  <div className="flex items-center gap-2 bg-jago text-white p-1.5 rounded-xl shadow-xs border border-jago hover:bg-jago-hover group transition-all min-w-0">
                    {/* Clickable Visual Sending Avatar Photo (Click directly on image to change) */}
                    <label 
                      className="relative shrink-0 cursor-pointer group/avatar rounded-full overflow-hidden border border-white/60 shadow-xs bg-slate-900 w-9 h-9 sm:w-11 sm:h-11 flex items-center justify-center p-0.5 hover:ring-2 hover:ring-amber-300 transition-all"
                    ...
                    <div className="flex flex-col flex-1 min-w-0 pr-1">
                      <span className="text-[6.5px] sm:text-[7px] font-black text-white/80 uppercase tracking-wider truncate">
                        Pengirim: {smtpConfig.fromName || "Tanpa Nama"}
                      </span>
                      <span className="text-[9.5px] sm:text-[10px] font-black text-white truncate break-all">
                        {smtpConfig.username}
                      </span>
                    </div>
                    <div className="flex items-center gap-1 bg-white/20 px-1.5 py-0.5 rounded-lg border border-white/30 shrink-0">
                      <div className="w-1.5 h-1.5 bg-emerald-300 rounded-full animate-pulse" />
                      <span className="text-[7.5px] sm:text-[8px] font-bold text-white uppercase">
                        Relay
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-2 bg-slate-50/50 p-1.5 rounded-xl border border-slate-200 border-dashed justify-center">
                    ...
                    <span className="text-[9px] font-bold text-slate-400 uppercase tracking-wide italic truncate">
                    ...
                    <div className="space-y-1">
                      ...
                      {parsedRecipients.length > 0 && !validationAnalysis.isValidAll && (
                        <motion.div
                          ...
                          className="p-2.5 sm:p-3 bg-rose-50/95 border border-rose-200 rounded-xl space-y-2 shadow-xs text-xs shrink-0"
                        >
                          <div className="flex items-center justify-between gap-2">
                            <span className="font-extrabold text-rose-700 uppercase tracking-wider text-[9.5px] sm:text-[10px] flex items-center gap-1.5 shrink-0">
                              <ShieldAlert className="w-3.5 h-3.5 text-rose-600 animate-pulse shrink-0" />
                              SMART BOUNCE GUARD WARNING
                            </span>
                            <span className="text-[8.5px] sm:text-[9px] font-bold text-rose-600 bg-rose-100 px-2 py-0.5 rounded-full shrink-0">
                              Format / Typo Dideteksi
                            </span>
                          </div>

                          {validationAnalysis.invalidList.length > 0 && (
                            <div className="text-[10.5px] sm:text-[11px] text-rose-800 font-semibold leading-relaxed break-all">
                              ⚠️ Format email tidak valid (sintaks): <strong className="font-mono text-rose-900 bg-white/80 px-1.5 py-0.5 rounded border border-rose-200 inline-block mt-0.5">{validationAnalysis.invalidList.join(", ")}</strong>
                            </div>
                          )}

                          {validationAnalysis.typoList.length > 0 && (
                            <div className="space-y-1.5 pt-1.5 border-t border-rose-200/60">
                              ...
                              {validationAnalysis.typoList.map((item, idx) => (
                                <div key={idx} className="flex flex-col xs:flex-row items-start xs:items-center justify-between bg-white/90 p-2 rounded-lg border border-rose-200 gap-1.5 sm:gap-2">
                                  <div className="font-mono text-[10.5px] sm:text-[11px] text-slate-700 break-all min-w-0 flex-1">
                                  ...
                                  <button
                                    ...
                                    className="px-2.5 py-1 bg-emerald-600 hover:bg-emerald-700 text-white text-[9.5px] font-extrabold rounded-md shadow-2xs transition-all uppercase tracking-wider shrink-0 cursor-pointer self-end xs:self-auto"
                                  >
                                    Perbaiki
                                  </button>
                                </div>
